...
//Overloading square brackets Note that this uses zero based indexing 
...
//Read-only variant of [], Note that this also uses zero based indexing
//and a check on the vlidity of the index
...
//Pverloading round breackets. Note that this uses "one based" indexing
//and a check on the validity of the index
double& Vector::operator() (int i) {
    assert(i > 0);
    assert(i < mSize + 1);
